...
void solve(LL tc)
{
	...
	VL coins(k);
	f(i,0,k){
		cin>>coins[i];	//array for storing different values of coins
	}
	dp[0]=0;
	fe(i,1,n){
		dp[i]=INT_MAX;
		for(auto it:coins){
			if(i-it>=0){
				dp[i]=min(dp[i],dp[i-it]+1);	//storing the minimum possible combination for that particular i
			}
		}
	}
	cout<<dp[n];	//Finally printing the minimum number of coins required to sum up amount n
}
int main()
{
        fastIO
